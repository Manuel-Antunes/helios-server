...
# **********************************************************************
# ALTRADO EM 08/06/2021 PARA EVITAR QUE O ELEITOR VOTE MAIS DE UMA VEZ
      if voter.vote_tinyhash != None:
            base_html = settings.HELIOS_TEMPLATE_BASENONAV  #Variável que armazena o template base. Como foi usado o render, o template não recuperou  a variável do settings.
            return render(request, 'election_ja_votou.html', locals())
# FIM DA ALTERAÇÃO
#***********************************************************************
...
def voters_email(request, election):
  ...
  default_body = render_template_raw(None, 'email/%s_body.txt' % template, {
      'election_name' : election.name,
      'election_voting_starts_at' : election.voting_starts_at.strftime('%d/%m/%Y %H:%M'),
      'election_voting_ends_at' : election.voting_ends_at.strftime('%d/%m/%Y %H:%M'),
